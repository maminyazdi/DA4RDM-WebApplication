...
def generate_xes_log(file_path, separator):       ###dataframe, case_id_col, activity_col, timestamp_col, cost_col):
    ...
    # import pandas as pd
    # dataframe['time:timestamp'] = pd.to_datetime(dataframe['time:timestamp'], format="%Y-%m-%d %H:%M:%S.%f")
    ...
    #log_csv = dataframe_utils.convert_timestamp_columns_in_df(log_csv, timest_format="%Y-%m-%d %H:%M:%S.%f", timest_columns=["time:timetamp"])
    log_csv['time:timestamp'] = pd.to_datetime(log_csv['time:timestamp'])

    log_csv = log_csv.sort_values('time:timestamp')
    print(log_csv["time:timestamp"].dtypes)
